...
from config.config_all import (
    OURS_SHOP, COMPLETE_RATE, ASSETS, TRADE_TYPES, FILTER_LIMIT_PRICE_VND)
...
async def fetch_p2p_binance(session, page: int = 1, trade_type: str = None, asset: str = None):
    try:
...
async def fetch_all_p2p_binance(trade_type, asset):
    async with aiohttp.ClientSession() as session:
        try:
            ...
            tasks = [
                (fetch_p2p_binance(session, page=page,
                 trade_type=trade_type, asset=asset))
...
async def get_p2p_binance(trade_type: str, asset: str):
    try:
        ...
        result["advertiser.nickName"] = result["advertiser.nickName"].astype(
            str)
        result["advertiser.userType"] = result["advertiser.userType"].astype(
            str)
        result["advertiser.monthFinishRate"] = result["advertiser.monthFinishRate"].astype(
            float)
        result["adv.price"] = result["adv.price"].astype(float)
        result["adv.minSingleTransAmount"] = result["adv.minSingleTransAmount"].astype(
            float)
        result["adv.dynamicMaxSingleTransAmount"] = result["adv.dynamicMaxSingleTransAmount"].astype(
            float)
        result["advertiser.page"] = result["advertiser.page"].astype(
            float).ffill()
...
async def final_price_binance_vnd():
    try:
        period = get_day_or_night()
        results = []

        for _, row in DataFrame(list(product(ASSETS, TRADE_TYPES)), columns=["asset", "trade_type"]).iterrows():
            p2p_data = await get_p2p_binance(trade_type=row.trade_type.upper(), asset=row.asset.upper())

            if p2p_data.empty:
                logger.danger(
                    f"Can't Get Binance P2P Data: {row.asset.lower()}_{row.trade_type.lower()}_{period}_vnd")
                continue

            p2p_data = p2p_data.loc[(~p2p_data["advertiser.nickName"].isin(OURS_SHOP))
            ...
            p2p_data_row_1, p2p_data_row_2 = get_filter_price_binance(
                FILTER_LIMIT_PRICE_VND[row.asset][period], p2p_data)
            ...
            values = {
                ...
                "min_amount_order": float(p2p_data_row_1["adv.minSingleTransAmount"].iloc[0]),
                "max_amount_order": float(p2p_data_row_1["adv.dynamicMaxSingleTransAmount"].iloc[0]),
                "complete_rate": float(p2p_data_row_1["advertiser.monthFinishRate"].iloc[0]),
            }
